const $=id=>document.getElementById(id),ids=['baseline','balanced','metric'],names=['Before native refit','Balanced / smoother','Metric-leaning'];
...
for(let i=0;i<3;i++)$('panels').insertAdjacentHTML('beforeend',`<section class="panel" id="panel${i}"><header><h2 id="title${i}">${names[i]}</h2><button class="select-panel" data-select="${i}">Pick here</button></header><div class="score" id="score${i}"></div><div class="vertex"><span>White</span><span>Vivid →</span></div><div class="canvas-wrap"><canvas class="triangle" id="tri${i}" tabindex="0" aria-label="${names[i]} color triangle"></canvas><span class="cursor" id="cursor${i}" hidden></span></div><div class="vertex"><span>Black</span></div><p class="gamut" id="gamut${i}"></p><div class="reading" id="read${i}">Move over the triangle to inspect a color; click to select it.</div><div class="barlabel" id="rlabel${i}"></div><canvas class="bar" id="reach${i}"></canvas><div class="barlabel" id="llabel${i}"></div><canvas class="bar" id="level${i}"></canvas><div class="panel-actions"><button data-save="${i}">Save triangle PNG</button><span class="hint" id="modelTag${i}"></span></div></section>`);
...
function updateControls(){
 ...
 for(let i=0;i<3;i++){$('panel'+i).classList.toggle('selected',ids[i]===state.checkpoint);$('title'+i).textContent=i===0?(state.gamut==='srgb'?'Previous native C1':'Corrected solid, before refit'):names[i];$('modelTag'+i).textContent=state.gamut==='srgb'?'native sRGB':'full gamut';}
 $('displayNotice').innerHTML=state.gamut==='srgb'?'<strong>Native sRGB, not a clipped full triangle.</strong> Every valid address is generated inside sRGB using this profile’s own vivid boundary and fitted R/L progression. The full-domain checkpoints remain separately available.':'<strong>Your display cannot show the whole full gamut.</strong> Actual full-domain XYZ is explicitly clipped to sRGB for preview only. The mask exposes those pixels. The underlying coordinates and benchmark distances do not use that clipped preview.';
 moveCursors();
}
...
worker.onmessage=({data:d})=>{
 ...
 if(d.done){$('status').textContent=`Ready · ${state.gamut==='srgb'?'native sRGB':'full gamut'} · balanced and metric refits loaded.`;return;}
 images.set(d.index,d);drawTriangle(d.index);paint($('reach'+d.index),d.reach);paint($('level'+d.index),d.level);
...
function colorRecord(){if(!lastSample||lastSample.gamut!==state.gamut||lastSample.sampleId!==sampleId)return null;const item=lastSample.readings[ids.indexOf(state.checkpoint)];return{profile:'HRL-0.9-dual-gamut-refits',gamut:state.gamut,checkpoint:state.checkpoint,referenceWhite:'D65',referenceWhiteNits:300,coordinates:lastSample.q,relativeXYZ:item.xyz,displaySRGB8:item.rgb,displayClipped:item.outside};}
...
fetch('research/native-srgb-refit/results/site-data.json').then(r=>{if(!r.ok)throw Error('Evidence unavailable: '+r.status);return r.json();}).then(d=>{evidenceData=d;updateEvidence();}).catch(e=>{$('progress').textContent=e.message;});
