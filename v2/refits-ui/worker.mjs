...
async function models(gamut){
 if(!cache.has(gamut))cache.set(gamut,Promise.all(ids.map(checkpoint=>createHRLRefits({gamut,checkpoint,overflow:'reject',imaginary:'reject'}))).catch(e=>{cache.delete(gamut);throw e;}));
...
function strip(model,H,axis,fixed){
 ...
 for(let x=0;x<width;x++){
  const t=x/(width-1),q=axis==='reach'?{H,R:fixed*t,L:fixed}:{H,R:fixed,L:fixed+(1-fixed)*t},out=display(model.toXYZ(q));
...
async function run(){
 ...
 try{while(latest){
  ...
  for(let i=0;i<ms.length;i++){
   ...
   self.postMessage({type:'render',id:job.id,gamut:job.gamut,H:job.H,index:i,tri,reach,level},[tri.pixels.buffer,tri.xyzs.buffer,tri.mask.buffer,reach.pixels.buffer,level.pixels.buffer]);
  }
