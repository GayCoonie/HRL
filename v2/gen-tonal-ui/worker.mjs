...
const ids=['old-balanced','old-metric','balanced','metric'],cache=new Map();
async function models(gamut){
 if(!cache.has(gamut))cache.set(gamut,Promise.all(ids.map(checkpoint=>checkpoint.startsWith('old-')?createSharedHRL({gamut,checkpoint:checkpoint.slice(4),overflow:'reject',imaginary:'reject'}):createGenTonalHRL({gamut,checkpoint,overflow:'reject',imaginary:'reject'}))).catch(e=>{cache.delete(gamut);throw e;}));
