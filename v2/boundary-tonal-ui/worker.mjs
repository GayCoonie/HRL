...
import {createShortCodeCodec} from '../codes.mjs';
...
const codecs=new WeakMap();async function codecFor(m){if(!codecs.has(m))codecs.set(m,createShortCodeCodec(m));return codecs.get(m);}
const ids=['parent','boundary','balanced','metric'],models=new Map();
function load(gamut,id,release=false){const key=gamut+':'+id+':'+release;if(!models.has(key))models.set(key,id==='parent'?createMappedHRL({gamut,checkpoint:'balanced'}).then(m=>{m.checkpoint='balanced';return m;}):release&&id==='metric'?createHRLv2({gamut}):createSpectralTonalHRL({gamut,checkpoint:id}));return models.get(key);}
...
let commandBusy=false;async function runCommands(){if(commandBusy)return;commandBusy=true;while(commands.length){const job=commands.shift();try{const m=await load(job.gamut,job.id,job.codes===true);if(job.type==='sample'){const xyz=m.toXYZ(job.q),extra={};if(job.codes){const codec=await codecFor(m);extra.shortCode=codec.serialize(job.q);extra.canonical=codec.toCanonical(job.q);extra.quantization=codec.quantize(job.q); }postMessage({type:'sampled',request:job.request,id:job.id,q:job.q,xyz,...extra,...display(xyz),gamut:job.gamut});}else if(job.type==='code'){const codec=await codecFor(m),q=codec.decode(job.code);postMessage({type:'imported',request:job.request,id:job.id,q,gamut:job.gamut});}else if(job.type==='import'){const text=job.hex.trim().replace(/^#/,'');if(!/^[a-f\d]{6}$/i.test(text))throw Error('Use a six-digit sRGB hex value, such as #8055CC.');const rgb=[0,2,4].map(i=>parseInt(text.slice(i,i+2),16)/255),xyz=mul3(SRGB_TO_XYZ,rgb.map(decodeSRGB)),q=m.fromXYZ(xyz);postMessage({type:'imported',request:job.request,id:job.id,q,gamut:job.gamut});}}catch(e){postMessage({type:'error',operation:job.type,request:job.request,message:e.message||'Unable to calculate this color.'});}}commandBusy=false;}
