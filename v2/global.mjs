...
const labels = { beta1: 'HRL v2 Beta 1', 'metric-global': 'Global metric', 'uniform-global': 'Global uniform', 'fresh-global': 'Fresh global' };
...
const recordURL = id => new URL(id === 'beta1' ? './research/boundary-tonal/results/metric.json' : `./research/global-tonal/trials/${id}.json`, import.meta.url);
...
function startWorker() {
  const records = new Map(), models = new Map(), codecs = new WeakMap();
  async function codecFor(model) { if (!codecs.has(model)) codecs.set(model, createShortCodeCodec(model)); return codecs.get(model); }
  ...
  async function sample(job) {
    ...
    try {
      ...
      for (const slot of ['beta1', 'candidate']) {
        ...
        if (slot === 'candidate') {const codec = await codecFor(model); colors[slot].shortCode = codec.serialize(job.q); colors[slot].canonical = codec.toCanonical(job.q); colors[slot].quantization = codec.quantize(job.q);}
      }
  ...
  self.onmessage = ({ data }) => {
    ...
    else if (data.type === 'decode') {newestSample=data.request;(async()=>{try{const model=await loadModel(data.gamut,data.candidate),codec=await codecFor(model),q=codec.decode(data.code);if(data.request===newestSample)self.postMessage({type:'decoded',request:data.request,q,gamut:data.gamut});}catch(error){self.postMessage({type:'error',operation:'decode',request:data.request,message:error.message});}})();}
  };
}

function startPage() {
  const $ = id => document.getElementById(id);
  const state = { gamut: 'srgb', candidate: 'metric-global', H: 30, R: .24, L: .25, mask: false };
  ...
  function syncControls() {
    ...
    const max=canonicalScale(state.gamut).maximum;
    for(const id of ['reach','level']){$(id).max=max;$(id).step='.01';}
    $('reach').value = (state.R*max).toFixed(2);
    $('level').value = (state.L*max).toFixed(2);
    for(const e of document.querySelectorAll('.scale-maximum'))e.textContent=max;
    ...
    $('coordinate-reading').textContent = `H ${state.H.toFixed(2)}° · R ${(state.R*max).toFixed(2)} · L ${(state.L*max).toFixed(2)}`;
  }
  function clearSamples() {
    currentColor=null;$('export-color').disabled=true;$('short-code').value='';
    for (const slot of ['beta1', 'candidate']) {
  ...
  function startEngine() {
    ...
    engineFailure = null;
    try {
      worker = new Worker(new URL('./global.mjs', import.meta.url), { type: 'module' });
      worker.onmessage = ({ data }) => {
        if (data.type === 'sheet' && data.request === renderRequest) {
          ...
          currentColor={...data.colors.candidate,coordinates:{H:state.H,R:state.R,L:state.L},gamut:state.gamut,candidate:state.candidate};$('short-code').value=currentColor.shortCode.code;$('export-color').disabled=false;
          for (const slot of ['beta1', 'candidate']) {
        ...
        } else if (data.type === 'decoded' && data.request === sampleRequest) { state.gamut=data.gamut;Object.assign(state,data.q);$('code-status').textContent='Code loaded in '+data.gamut+'.';render();fillTable();
        } else if (data.type === 'error') {
          if(data.operation==='decode'&&data.request===sampleRequest){$('code-status').textContent=data.message;render();}
  ...
  const metricRows = [
    ...
    ['All-hue sheet bending · mean', 'tonal', 'sheetMean'],
    ['All-hue sheet bending · RMS', 'tonal', 'sheetRMS'],
    ['Worst hue · mean sheet bending', 'tonal', 'sheetWorstHue'],
  ...
  function fillTable() {
    ...
    $('selection-note').textContent = receipt.selectionNote;
  ...
  async function loadResults() {
    ...
    receipt = null;
    try {
      const response = await fetch(new URL('./research/global-tonal/results/comparison.json', import.meta.url), { cache: 'no-cache' });
      if (!response.ok) throw Error(`HTTP ${response.status}`);
      const result = await response.json();
      if (result.schemaVersion !== 1 || !result.models || typeof result.models !== 'object' || !safeCandidate(result.selectedCandidate) || result.status !== 'research-comparison' || typeof result.selectionNote !== 'string') throw Error('unrecognized comparison record');
  ...
  $('reach').onchange = () => setCoordinates($('reach').valueAsNumber/canonicalScale(state.gamut).maximum, state.L);
  $('level').onchange = () => setCoordinates(state.R, $('level').valueAsNumber/canonicalScale(state.gamut).maximum);
  ...
  $('import-code').onclick=()=>{const code=$('short-code').value.trim().toUpperCase(),gamut=code.length===9?'srgb':code.length===12?'full':null;if(!gamut){$('code-status').textContent='Use a nine-character sRGB or twelve-letter full code.';return;}renderRequest++;sampleRequest++;clearSamples();worker?.postMessage({type:'decode',request:sampleRequest,gamut,candidate:state.candidate,code});};
  $('short-code').onkeydown=e=>{if(e.key==='Enter')$('import-code').click();};
  $('export-color').onclick=()=>{if(!currentColor)return;const data={format:'HRL-v2-color',coordinates:currentColor.coordinates,canonicalCoordinates:currentColor.canonical,shortCode:currentColor.shortCode,codeQuantization:currentColor.quantization,relativeXYZ:currentColor.xyz,gamut:state.gamut,candidate:state.candidate,displayClipped:currentColor.clipped},url=URL.createObjectURL(new Blob([JSON.stringify(data,null,2)],{type:'application/json'})),a=document.createElement('a');a.href=url;a.download=`hrl-${state.candidate}-${state.gamut}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),2000);};
  $('share-view').onclick=async()=>{const url=new URL(location.href);url.hash=new URLSearchParams({g:state.gamut,c:state.candidate,h:state.H,r:state.R,l:state.L});try{await navigator.clipboard.writeText(url.href);$('code-status').textContent='View link copied.';}catch{$('code-status').textContent=url.href;}};
  function restore(){const p=new URLSearchParams(location.hash.slice(1));if(['srgb','full'].includes(p.get('g')))state.gamut=p.get('g');if(safeCandidate(p.get('c'))){state.candidate=p.get('c');candidateTouched=true;}if(p.has('h')&&Number.isFinite(+p.get('h')))state.H=((+p.get('h')%360)+360)%360;if(p.has('l')&&Number.isFinite(+p.get('l')))state.L=clamp(+p.get('l'),0,1);if(p.has('r')&&Number.isFinite(+p.get('r')))state.R=clamp(+p.get('r'),0,state.L);}
  restore();
  window.addEventListener('hashchange',()=>{restore();render();fillTable();});
  syncControls();
  startEngine();
  loadResults();
}
