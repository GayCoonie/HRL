...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--name',required=True);ap.add_argument('--start',default='../boundary-tonal/results/metric.json');ap.add_argument('--steps',type=int,default=350);ap.add_argument('--turn',type=float,default=1200);ap.add_argument('--order',type=float,default=100);ap.add_argument('--smooth',type=float,default=.001);ap.add_argument('--no-hue',action='store_true');ap.add_argument('--grid',choices=['fine','coarse'],default='fine');args=ap.parse_args()
 ...
 def assess():
  ...
  for p in profiles:
   ...
   loss=st+.1*uw+args.turn*nearpen+args.turn*.04*wholepen+args.smooth*hue_mean(rough)+args.order*ordering+.005*hue_mean(cv2)
