...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--name',required=True);ap.add_argument('--start',default='../boundary-tonal/results/metric.json');ap.add_argument('--steps',type=int,default=350);ap.add_argument('--turn',type=float,default=1200);ap.add_argument('--order',type=float,default=100);ap.add_argument('--smooth',type=float,default=.001);ap.add_argument('--no-hue',action='store_true');ap.add_argument('--speed',type=float,default=.005);ap.add_argument('--optimizer',choices=['lbfgs','adam','hybrid'],default='lbfgs');ap.add_argument('--lr',type=float,default=.0007);ap.add_argument('--adam-steps',type=int,default=150);ap.add_argument('--grid',choices=['fine','coarse'],default='fine');args=ap.parse_args()
 ...
 if rec.get('schema')=='hrl-joint-interior-v1':C,D,N,A=[torch.tensor(rec[k],dtype=torch.float64) for k in ['coefficients','dark','neutral','hue_coefficients']]
 ...
 for g,p in cache['profiles'].items():
  gp=O/'fine' if args.grid=='fine' else O;gr=json.loads((gp/f'grid-{g}.json').read_text());b=(gp/f'source-{g}.f64').read_bytes();assert hashlib.sha256(b).hexdigest()==gr['sha256'];hh=torch.tensor(gr['hues']+[360.]);n=gr['N'];a=np.frombuffer(b,dtype='<f8').copy().reshape(len(gr['hues']),n,n,4);a=np.concatenate([a,a[:1]],axis=0);im=torch.tensor(a).permute(3,0,1,2)[None].contiguous()
  pp={k:torch.tensor(p[k]) for k in ['a','b','dv','w']};pp.update(g=g,image=im,hs=hh,grid=gr);profiles.append(pp)
 hs=sorted(set(list(np.arange(0,360,2.5))+[289.5,290.57,291.5,294.,296.]))
 # Cyclic quadrature keeps targeted samples from giving violet extra mean weight.
 gaps=np.diff(np.r_[hs,hs[0]+360]);hue_weights=torch.tensor((gaps+np.roll(gaps,1))/720)
 def hue_mean(v):return (v.reshape(len(hs),-1).mean(-1)*hue_weights).sum()
 nearU=np.unique(np.r_[np.linspace(0,.25,65),.0001,.0005,.001,.0025]);nearL=[.005,.01,.02,.04,.08,.12,.2];wholeU=np.linspace(0,1,65);wholeL=[.02,.08,.2,.5,.8];orderT=np.unique(np.r_[np.linspace(0,1,65),.0005,.001,.0025,.005,.01]);rr=[.0001,.001,.01,.05,.2,.5,.8]
 ...
 def assess():
  ...
  for p in profiles:
   ...
   values=sample(b,p);parts=torch.split(values,lens);vn,vw,vo=[v.reshape(*shape[:-1],4) for v,shape in zip(parts,shapes)];tn=turn(vn);tw=turn(vw);nearpen=hue_mean(tn.square())+.03*tn.max().square();wholepen=hue_mean(tw.square())+.01*tw.max().square()
   ...
   neg=F.relu(vo[:,:-1,0]-vo[:,1:,0]);yp=F.relu(vo[:,:-1,3]-vo[:,1:,3]);ot=neg.sum(-1);yt=yp.sum(-1)
   # Integrated backtracking cannot disappear merely by splitting a bad step.
   ordering=hue_mean(ot.square())+.03*ot.max().square()+hue_mean(yt.square())
   speed=(vw[:,1:,:3]-vw[:,:-1,:3]).square().sum(-1).clamp_min(1e-30).sqrt();cv2=speed.var(-1,unbiased=False)/speed.mean(-1).square().clamp_min(1e-15)
   loss=st+.1*uw+args.turn*nearpen+args.turn*.04*wholepen+args.smooth*hue_mean(rough)+args.order*ordering+args.speed*hue_mean(cv2)
   total=total+.5*loss;stats[p['g']]={'weighted':float(100*st.clamp_min(0).sqrt().detach()),'unweighted':float(100*uw.clamp_min(0).sqrt().detach()),'nearMeanTurn':float(tn.mean().detach()),'nearWorstTurn':float(tn.max().detach()),'nearAbove001':int((tn>.001).sum().detach()),'nearPaths':tn.numel(),'wholeMeanTurn':float(tw.mean().detach()),'gradientVariation':float(rough.mean().detach()),'fixedRWorstDrop':float(neg.max().detach()),'fixedRMeanTotalRetreat':float(ot.mean().detach()),'fixedRWorstTotalRetreat':float(ot.max().detach()),'wholeMeanStepCV':float(cv2.sqrt().mean().detach())}
  total=total+1e-7*(C*freq).square().mean()+1e-5*A.square().mean()+1e-7*D.square().mean();return total,stats
 def save():
  if best is None:return
  loss,cs,ds,ns,As,stats,call=best;r={'schema':'hrl-joint-interior-v1','id':'HRL-joint-'+args.name,'variant':args.name,'harmonics':K,'hue_enabled':hue_enabled,'coefficients':cs.tolist(),'dark':ds.tolist(),'neutral':ns.tolist(),'hue_coefficients':As.tolist(),'gamut_calibration':'shared','ring_logits':None,'source':'deterministic-own-anchor-normalized','research':{'startSHA256':hashlib.sha256(raw).hexdigest(),'codeSHA256':codehash,'coreSHA256':hashlib.sha256((P/'core.py').read_bytes()).hexdigest(),'cacheSHA256':hashlib.sha256(cachepath.read_bytes()).hexdigest(),'fit':vars(args),'stats':stats,'bestLoss':loss,'bestCall':call,'calls':calls,'seconds':time.monotonic()-start,'trainingHues':hs,'hueWeights':hue_weights.tolist(),'meanLossWeighting':'cyclic Voronoi hue quadrature; record stats below remain unweighted sample means','orderingLoss':'sum of all negative J steps per path, squared mean plus .03 global max squared, and squared total negative Y travel','wholeSpeedCVWeight':args.speed,'nearLevels':nearL,'nearU':nearU.tolist(),'gridSHA256':{p['g']:p['grid']['sha256'] for p in profiles},'populations':{'srgb_retained':3331,'full':3813},'limits':['COMBVD training data','Approximate trilinear GenSpace regularizer; direct audits required','No new ColorBench run','Synthetic gradient/J diagnostics do not define HRL Level']}};(O/f'{args.name}.json').write_text(json.dumps(r,indent=2)+'\n')
 params=[C,D,N]+([A] if hue_enabled else []);optimizer=torch.optim.Adam(params,lr=args.lr) if args.optimizer!='lbfgs' else torch.optim.LBFGS(params,lr=.7,max_iter=args.steps,history_size=25,line_search_fn='strong_wolfe',tolerance_grad=2e-8,tolerance_change=1e-12)
 def closure():
  nonlocal calls,best
 ...
 if args.optimizer!='lbfgs':
  n=args.steps if args.optimizer=='adam' else min(args.adam_steps,args.steps)
  for i in range(n):
   optimizer.param_groups[0]['lr']=args.lr*(.15+.85*(1+math.cos(math.pi*i/max(1,n-1)))/2);closure();optimizer.step()
  closure() # Assess the nth update before saving or restoring the best state.
  if args.optimizer=='hybrid' and args.steps>n:
   with torch.no_grad():
    for x,v in zip([C,D,N,A],best[1:5]):x.copy_(v)
   optimizer=torch.optim.LBFGS(params,lr=.7,max_iter=args.steps-n,history_size=25,line_search_fn='strong_wolfe',tolerance_grad=2e-8,tolerance_change=1e-12);optimizer.step(closure)
 else:optimizer.step(closure)
 save();trace.close();print('FINISHED',args.name,json.dumps(best[-2]),flush=True)
