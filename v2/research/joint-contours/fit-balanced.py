...
def main():
 ...
 def assess():
  ...
  for p in profiles:
   ...
   neg=F.relu(vo[:,:-1,0]-vo[:,1:,0]);yp=F.relu(vo[:,:-1,3]-vo[:,1:,3]);ordering=hue_mean(neg.square())+.01*neg.max().square()+hue_mean(yp.square())
   loss=st+.1*uw+args.turn*nearpen+args.turn*.04*wholepen+args.smooth*hue_mean(rough)+args.order*ordering
