...
const args=process.argv.slice(2),names=args.length?args:['old-balanced','balanced','metric'];
...
const out={domain:gamut+' own source chart; all metrics on XYZ, never display-clipped',method:'Actual JS toXYZ -> Oklab, constant public Reach and Level paths. Float XYZ, exclude three endpoint steps. Training used a separate interpolated surrogate.',hue_samples:nh,hue_offset_degrees:360/nh/2,steps,models:{}};
for(const name of names){
 const m=name==='old-balanced'?await createHRLRefits({gamut,checkpoint:'balanced'}):await createSharedHRL({gamut,checkpoint:name});
 ...
 fs.writeFileSync(new URL(`results/visual-${gamut}-${dense?'dense':'screen'}-${names.join('-')}.json`,import.meta.url),JSON.stringify(out,null,2)+'\n');
}
