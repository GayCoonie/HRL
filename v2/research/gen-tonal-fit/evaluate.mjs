...
import {createGenTonalHRL} from './index.mjs';import {createSharedHRL} from '../shared-rl/source.mjs';
...
const cache=JSON.parse(fs.readFileSync(new URL('../shared-rl/results/cache.json',root))),out={hues:HCOUNT,samples:N,hueOffset:offset,modelBased:true,method:'Actual inverse XYZ and frozen 3D GenSpace. No display clipping. Two endpoint steps trimmed for CV/jump only.',models:{}};
for(const name of names){out.models[name]={};for(const gamut of ['srgb','full']){
 const file=name.startsWith('old-')?`../shared-rl/results/${name.slice(4)}.json`:`${fs.existsSync(new URL('results/'+name+'.json',root))?'results':'trials'}/${name}.json`,raw=fs.readFileSync(new URL(file,root)),record=JSON.parse(raw);
 const m=name.startsWith('old-')?await createSharedHRL({gamut,record}):await createGenTonalHRL({gamut,record});const rows=[];
 ...
 const p=cache.profiles[gamut];let dd=0,dv=0,vv=0,dd0=0,dv0=0,vv0=0;
