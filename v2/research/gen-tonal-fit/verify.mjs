...
import {createGenTonalHRL,ADOBE_RGB1998,coordinates} from './index.mjs';
...
for(const name of names){
 ...
 for(const gamut of ['srgb','full']){
  const m=await createGenTonalHRL({gamut,record:rec,overflow:'reject',imaginary:'reject'}),old=await createSharedHRL({gamut,checkpoint:'balanced'});ms.push(m);let error=0,neutral=0,hue=0,minDet=Infinity,maxCondition=0,minChannel=Infinity,maxChannel=-Infinity;
 ...
 const third=await createGenTonalHRL({gamut:ADOBE_RGB1998,record:rec});let e=0;for(let i=0;i<2048;i++){const rgb=[rand(),rand(),rand()],q=third.fromLinearRGB(rgb),back=third.toLinearRGB(q);e=Math.max(e,maxerr(rgb,back));}assert(e<1e-8);assert.deepEqual(third.definition.coefficients,rec.coefficients);out.untrainedThirdGamut={id:'a98-rgb',samples:2048,maxLinearRGBError:e,refitted:false};
 const s100=await createGenTonalHRL({gamut:'srgb',record:rec,referenceWhiteNits:100});for(let i=0;i<1024;i++){const rgb=[rand(),rand(),rand()];assert.deepEqual(s100.fromRGB(rgb),ms[0].fromRGB(rgb));}out.relative100vs300IdentitySamples=1024;
