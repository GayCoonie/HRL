"""Beta 1 continuation with per-hue risk and equilateral sheet regularity.
All path and sheet penalties are synthetic GenSpace diagnostics, not observer data.
"""
from pathlib import Path
import argparse,copy,json,time,hashlib,math,importlib.util,sys,os
import numpy as np
import torch
...
def main():
    ...
    for key,default,typ in [('name',None,str),('start','metric-b2',str),('steps',700,int),('stride',2,int),('samples',97,int),('visual',.28,float),('fair',.6,float),('sheet',.08,float),('corner',1.,float),('guard',2.,float),('blue',.75,float),('layers',7,int),('mode','bicubic',str),('gridroot','results',str),('metricguard',20000.,float),('stressslack',.05,float),('pathguard',20.,float),('conditioning',.002,float),('conditionlimit',40.,float),('unweighted',.1,float),('outdir','trials',str)]:
        ap.add_argument('--'+key,default=default,type=typ,required=key=='name')
    ap.add_argument('--sheetmetric',choices=['extended-robust','raw-core','robust-core'],default='extended-robust')
    ap.add_argument('--edgeguard',type=float,default=0.)
    args=ap.parse_args();output=(O/args.outdir).resolve();output.mkdir(parents=True,exist_ok=True)
    if not args.name or any(c not in 'abcdefghijklmnopqrstuvwxyz0123456789-' for c in args.name):raise ValueError('Use a unique lowercase trial name')
    if any((output/f'{args.name}.{ext}').exists() for ext in ['json','jsonl']):raise FileExistsError('Refusing to overwrite a trial')
    torch.set_default_dtype(torch.float64);torch.set_num_threads(1);torch.manual_seed(180917);np.random.seed(180917);torch.use_deterministic_algorithms(True)
    parentpath=P/'trials/metric-b2.json';parent=json.loads(parentpath.read_text());startpath=parentpath if args.start=='parent' else Path(args.start) if Path(args.start).is_absolute() else P/f'trials/{args.start}.json';rec=json.loads(startpath.read_text())
    ...
    gridpath=P/args.gridroot/'grid.json';grid=json.loads(gridpath.read_text());HH,N=grid['H'],grid['N'];baseH=grid.get('baseH',HH);hh=list(range(0,baseH,args.stride))+list(range(baseH,HH));hs=torch.tensor(grid.get('hues',[i*360/HH for i in range(HH)]))[hh];NH=len(hh)
    ...
    historical=[(i/32,j/32) for j in range(4,31,2) for i in range(2,j-1,2)]+[(L*u,L) for L in [.04,.08,.12,.18] for u in [.18,.4,.65,.84]]
    coremask=torch.tensor([point in historical for point in centres],dtype=torch.bool)
    assert int(coremask.sum())==121 and int((~coremask).sum())==20
    ...
    def sheet_terms(v):
        ...
        robust=bend[:,coremask] if args.sheetmetric=='raw-core' else torch.log1p(bend[:,coremask] if args.sheetmetric=='robust-core' else bend)
    ...
    for g in ['srgb','full']:
        ...
        with torch.no_grad():
            pv=sample(q,pc,pd,image);p['base_risk'],_,p['base_edge'],_=path_terms(pv);p['base_sheet'],p['base_bending']=sheet_terms(sample(sq,pc,pd,image))
    ...
    condition_hues=sorted(set(hs.tolist()+[263,269,273,275,277,281,285,293]))
    cq=torch.tensor([[H,L*U,L] for H in condition_hues for L in [.001,.005,.02,.05,.1,.25,.5,.8,.95,.995] for U in [.005,.05,.2,.5,.8,.95,.99,.999]])
    with torch.no_grad():
    ...
    def assess():
        ...
        cp=F.relu(lc-condition_limit).square()
        cw=1+2*torch.exp(-.5*((((cq[:,0]-273+180)%360-180)/20).square()))
        cw=cw/cw.mean()
        conditioning=(cp*cw).mean()+.2*torch.sqrt((cp.square()*cw).mean()+1e-20)
        for p in profiles:
            ...
            regret=F.relu(risk-1.02*p['base_risk']-.001).square().mean()
            metric_limit=(p['base_stress2'].sqrt()+args.stressslack/100).square()
            metric_guard=F.relu(st-metric_limit).square()
            edge_regret=F.relu(torch.log1p(bending[:,~coremask])-torch.log1p(p['base_bending'][:,~coremask])-math.log(1.05)).square().mean()
            unweighted=1-(de*dv).sum().square()/((de*de).sum()*(dv*dv).sum())
            loss=st+args.unweighted*unweighted+args.visual*riskagg(risk)+args.sheet*riskagg(sheet)+args.corner*retreat+args.guard*guard+args.pathguard*regret+args.metricguard*metric_guard+args.edgeguard*edge_regret
            total=total+.5*loss
            stats[p['gamut']]={k:float(x.detach()) for k,x in {'stress':100*st.clamp_min(0).sqrt(),'unweighted':100*unweighted.clamp_min(0).sqrt(),'conditionPenalty':conditioning,'maxLogCondition':lc.max(),'pathRisk':risk.mean(),'worstHue':risk.max(),'bluePathRisk':risk[((hs-273+180)%360-180).abs()<23].mean(),'sheetBending':sheet.mean(),'worstSheet':sheet.max(),'blueSheet':sheet[((hs-273+180)%360-180).abs()<23].mean(),'retreat':retreat,'darkGuard':guard,'regret':regret,'edgeRegret':edge_regret,'metricGuard':metric_guard,'metricLimitSTRESS':100*metric_limit.sqrt()}.items()}
    ...
    def save(best):
        loss,cc,dd,stats,evaluation=best;r=copy.deepcopy(rec);r.update(id='HRL-v2-tonal-next-'+args.name,variant=args.name,layers=args.layers,coefficients=[cc.tolist()]);r['dark']['coefficients']=dd.tolist()
        r['research']={'ruler':'helmlab-1.0.0-genspace','human_training':['COMBVD'],'parent':'2.0.0-beta.1 metric-b2; accuracy and path guard reference','parent_sha256':hashlib.sha256(parentpath.read_bytes()).hexdigest(),'start':os.path.relpath(startpath,P),'start_sha256':hashlib.sha256(startpath.read_bytes()).hexdigest(),'fit':vars(args),'grid_record':grid,'seed':180917,'per_gamut_objective_weights':[.5,.5],'synthetic':'Analytic conditioning tail, weighted/unweighted COMBVD, per-hue weighted mean/RMS path risk; equilateral Hessian/gradient bending; black/white corner retreats; parent dark-edge and good-hue guards','sheet_stencils':len(centres),'sheet_reference_scale':1/32,'sheet_penalty_transform':args.sheetmetric,'core_sheet_stencils':121,'extra_edge_stencils':20,'edge_guard_log_slack':math.log(1.05),'blue_window':{'centre':273,'sigma':20,'amplitude':args.blue,'basis':'User-reported region, not observer dataset'},'stats':stats,'best_loss':loss,'best_evaluation':evaluation,'evaluations':calls,'seconds':time.monotonic()-start,'versions':{'torch':torch.__version__,'numpy':np.__version__},'boundary':'cie1931-2deg-1nm-linear-cone','conditioning':'Analytic common-map Jacobian; mean/RMS log-condition exceedance, blue weight and exact critical-angle stencil shared across gamuts','old_lightness_power_target':False, 'code_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'data_exposure':'Prior development exposed evaluation datasets; no new held-out result used in this loss.'}
    ...
    def closure():
        nonlocal calls,best
        opt.zero_grad();loss,stats=assess();assert torch.isfinite(loss);loss.backward();assert torch.isfinite(C.grad).all() and torch.isfinite(D.grad).all();calls+=1
