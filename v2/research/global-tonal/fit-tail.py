"""Adaptive raw-tail whole-solid optimization after measured uniform-global tail regression.
...
def main():
    ...
    ap.add_argument('--start',choices=['beta1','fresh','uniform'],default='uniform')
    ...
    parentpath=P/'results/metric.json';startpath=O/'trials/uniform-global.json' if args.start=='uniform' else parentpath;rec=json.loads(startpath.read_text());assert rec['neutral_shift']==json.loads(parentpath.read_text())['neutral_shift'];gridroot=Path(args.gridroot);grid=json.loads((gridroot/'grid.json').read_text())
    ...
    centres=[(i/32,j/32) for j in range(4,31,2) for i in range(2,j-1,2)]+[(L*u,L) for L in [.04,.08,.12,.18] for u in [.18,.4,.65,.84]]
    ...
    def sheet_terms(v):
        ...
        return bending.mean(-1)+.25*(bending.square().mean(-1)+1e-20).sqrt(),bending
    ...
    def assess():
        ...
        for p in profiles:
            ...
            total=total+.5*loss;stats[p['gamut']]={k:float(v.detach()) for k,v in {'stress':100*st.clamp_min(0).sqrt(),'unweighted':100*uw.clamp_min(0).sqrt(),'pathRisk':risk.mean(),'worstHuePathRisk':risk.max(),'sheetRawMeanRMS':sheet.mean(),'sheetRaw':bend.mean(),'worstHueSheetRaw':bend.mean(-1).max(),'retreat':retreat,'maxLogCondition':lc.max(),'conditionPenalty':condition}.items()}
