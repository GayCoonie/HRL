...
import fs from 'node:fs';import {createRelativeHRL} from '../relative-domain/index.mjs';import {createHRLRefits} from './index.mjs';
...
for(const name of names){
 const m=name==='baseline'?await createRelativeHRL({gamut:'srgb',variant:'candidate'}):await createHRLRefits({gamut:'srgb',checkpoint:name});let yd=0,ydmax=0,cd=0,cdmax=0,ls=0,rs=0;
