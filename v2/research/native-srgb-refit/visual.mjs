...
const args=process.argv.slice(2),names=args.length?args:['baseline','balanced','metric'];
...
const out={domain:'Native sRGB, own source atlas and vivid boundary; no output clipping',method:'Actual JS toXYZ -> Oklab, constant public Reach and Level paths. Float XYZ, exclude three endpoint steps. Training used a separate interpolated surrogate.',hue_samples:nh,hue_offset_degrees:360/nh/2,steps,models:{}};
for(const name of names){
 const m=name==='baseline'?await createRelativeHRL({gamut:'srgb',variant:'candidate',overflow:'reject',imaginary:'reject'}):await createHRLRefits({gamut:'srgb',checkpoint:name,overflow:'reject',imaginary:'reject'});
