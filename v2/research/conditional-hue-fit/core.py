"""Vectorized Python realization of hrl-conditional-hue-v4 for fitting.
...
def _delta(h, coeffs, xp):
    raw=xp.sum(_features(h,xp)*coeffs,axis=-1)
    return 25*xp.tanh(raw/25)


def _delta_prime(h, coeffs, xp):
    theta=h*(math.pi/180)
    raw=xp.sum(_features(h,xp)*coeffs,axis=-1)
    c1,s1,c2,s2=coeffs[1],coeffs[2],coeffs[3],coeffs[4]
    slope=(math.pi/180)*(-c1*xp.sin(theta)+s1*xp.cos(theta)-2*c2*xp.sin(2*theta)+2*s2*xp.cos(2*theta))
    tanh=xp.tanh(raw/25)
    return (1-tanh*tanh)*slope


def _hue_inverse(hphys,weight,coeffs,xp):
    # A fixed number of Newton steps supports autodiff; |coeff|<=8 guarantees
    # the one-to-one circle map's derivative exceeds ~0.162.
    h=hphys
    for _ in range(9):
        h=h-(h+weight*_delta(h,coeffs,xp)-hphys)/(1+weight*_delta_prime(h,coeffs,xp))
    return h
...
def public_to_source(q, record, xp=np):
    ...
    weight=4*a*s*(1-s)
    physical_h=h+weight*_delta(h,record['hue_coefficients'],xp)
    return xp.stack((physical_h, a*s, a), axis=-1)


def source_to_public(q, record, xp=np, iterations=13):
    """Triangular exact-CDF inverse for observer pairs."""
    physical_h, sr, a = q[..., 0], q[..., 1], q[..., 2]
    s = sr/xp.maximum(a, 1e-300)
    weight=4*a*s*(1-s)
    h=_hue_inverse(physical_h,weight,record['hue_coefficients'],xp)
    ld = _curve(h, record["level_logits"], record, xp,level_factor=s)
