"""Reproducible scratch fit of shared conditional positive-CDF coordinates.
...
BASE_GRID = Path('/workspace/scratch/f91573918531/fit-source/v2/research/fresh-field/results/base-grid')
...
def _sample_grid(source, q, xp):
    ...
    h_raw = np.asarray(getval(q[:,0]))
    h_value = np.mod(h_raw, 360)
    # Freeze only interpolation cell selection; keep the weight differentiable
    # through physical hue, including tonal dependence of the hue-flow weight.
    h = q[:,0]-360*np.floor(h_raw/360)
    ih = np.searchsorted(hues, h_value, side='right')-1
    ih = np.maximum(ih, 0)
...
def _record(vector, seed, xp, mode):
    ...
    rec['reach_level_logits']=xp.reshape(vector[25:50],(5,5)) if mode!='lift-only' else xp.zeros((5,5))
    rec['level_logits']=xp.reshape(vector[50:75],(5,5))
    rec['hue_coefficients']=vector[75:80]
    return rec
...
def _numbers(value):
    ...
    if hasattr(value,'shape') and value.shape:
        return [_numbers(x) for x in value]
    return float(value)
...
def main(argv=None):
    ...
    ap.add_argument('--mode',choices=['shared','lift-only','hue-only'],default='shared')
    ap.add_argument('--start',choices=['neutral','graded'],default='neutral')
    ap.add_argument('--start-record',type=Path,default=None)
    ap.add_argument('--holdout-weight-seven',action='store_true')
    ap.add_argument('--stage',choices=['visual','joint'],default='visual')
    ap.add_argument('--visual-weight',type=float,default=1.0)
    ap.add_argument('--pathguard',type=float,default=0.0,
                    help='Penalty on squared worst fixed-public-Reach GenSpace J retreat per gamut')
    ap.add_argument('--blue-contact',type=float,default=0.0)
    ap.add_argument('--yellow-contact',type=float,default=0.0)
    ...
    for key in ('reach_logits','reach_level_logits','level_logits','hue_coefficients'):
        seed[key]=np.array(seed[key],dtype=float)
    vector=np.concatenate((seed['reach_logits'].ravel(),seed['reach_level_logits'].ravel(),
                           seed['level_logits'].ravel(),seed['hue_coefficients'].ravel()))
    ...
    if args.mode=='lift-only':vector[:50]=0
    if args.start_record is not None:
        prior=json.loads(args.start_record.read_text())
        if prior['schema'] not in (seed['schema'],'hrl-conditional-field-v3') or prior['knots']!=seed['knots'] or prior['harmonics']!=seed['harmonics']:
            raise ValueError('Incompatible warm-start record')
        vector=np.concatenate(tuple(np.asarray(prior[k],dtype=float).ravel()
                      for k in ('reach_logits','reach_level_logits','level_logits')))
        vector=np.concatenate((vector,np.asarray(prior.get('hue_coefficients',[0]*5),dtype=float)))
    ...
    profiles_all={g:{k:np.asarray(v,dtype=float) for k,v in pairs[g].items() if k in ('a','b','dv','w')}
              for g in ('srgb','full')}
    heldout={g:{k:a[a7['w']==7] for k,a in a7.items()} for g,a7 in profiles_all.items()} if args.holdout_weight_seven else None
    profiles={g:{k:a[a7['w']!=7] for k,a in a7.items()} for g,a7 in profiles_all.items()} if args.holdout_weight_seven else profiles_all
    hues=np.arange(0,360,args.hue_stride).tolist()
    hues=sorted(set(hues+[55,65,140,180,230,270,287.5,289.5,292.5,295,320]))
    paths,shape=path_queries(hues)
    blue_hues=[269,273,277]
    blue_points=[(.1,.5,.35),(.08,.8,.22),(.07,.35,.28)]
    blue_queries=np.array([[H,R,L] for H in blue_hues for R,L,_ in blue_points],dtype=float)
    blue_targets=anp.array([s for H in blue_hues for _,_,s in blue_points])
    yellow_queries=np.array([[H,.8*.8,.8] for H in (115,120,125)],dtype=float)
    yellow_target=anp.array([.56,.58,.56])
    timer=time.monotonic();trace=[]
    def assess(x, diagnostics=False):
        rec=_record(x,seed,anp,args.mode)
        total=0;stats={}
        bp=public_to_source(blue_queries,rec,anp)
        blue_s=bp[:,1]/bp[:,2]
        blue_penalty=anp.mean(anp.maximum(blue_targets-blue_s,0)**2)
        yellow=_sample_grid(grids['srgb'],public_to_source(yellow_queries,rec,anp),anp)[:,3]
        yellow_penalty=anp.mean(anp.maximum(yellow_target-yellow,0)**2)
        for g in ('srgb','full'):
            ...
            if args.stage=='joint' or diagnostics:
                ...
                if heldout is not None and diagnostics:
                    p7=heldout[g]
                    qa=source_to_public(p7['a'],rec,anp)
                    qb=source_to_public(p7['b'],rec,anp)
                    hd=anp.sqrt(anp.sum((embed(qa,anp)-embed(qb,anp))**2,axis=-1)+1e-30)
                    stats[g]['heldoutWeightSevenSTRESS100']=100*anp.sqrt(anp.maximum(stress(hd,p7['dv'],p7['w'],anp),0))
            else:
                weighted=unweighted=0
                stats[g]=metrics
            hard_path=args.pathguard*metrics['fixedReachRetreatWorst']**2
            total=total+.5*(args.visual_weight*visual+hard_path+
                              (weighted+.15*unweighted if args.stage=='joint' else 0))
        # Penalize undulation and coefficient magnitude; terminal anchors stay exact.
        coeff=anp.reshape(x[:75],(3,5,5))
        frequency=anp.array([1.,1.5,1.5,2.5,2.5])
        regularizer=.00006*anp.mean((coeff*frequency)**2)+.00001*anp.mean(x[75:80]**2)
        total=total+regularizer+args.blue_contact*blue_penalty+args.yellow_contact*yellow_penalty
        if diagnostics:
            stats['contact']={'bluePhysicalS':blue_s,'yellowPhysicalY':yellow,
                              'bluePenalty':blue_penalty,'yellowPenalty':yellow_penalty}
    ...
    bounds=[(-4,4)]*75+[(-8,8)]*5
    if args.mode=='lift-only':bounds[:50]=[(0,0)]*50
    if args.mode=='hue-only':bounds[:75]=[(float(v),float(v)) for v in vector[:75]]
    ...
    rec['research']={'status':'experimental unvalidated fit; do not promote',
       ...
       'holdoutPopulation':({g:len(heldout[g]['a']) for g in heldout} if heldout is not None else None),
       'holdoutMethod':'multiplicity w=7 excluded from pair loss and held as development check' if heldout is not None else None,
       'freshStarts':args.start,'ablation':args.mode,'objectiveStage':args.stage,
       'startRecordSHA256':sha(args.start_record) if args.start_record is not None else None,
