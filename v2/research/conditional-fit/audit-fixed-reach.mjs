...
const bytes=fs.readFileSync(recordPath),record=JSON.parse(bytes),sha=x=>crypto.createHash('sha256').update(x).digest('hex');
...
const output={schema:'hrl-conditional-direct-fixed-reach-v1',recordPath,recordSHA256:sha(bytes),
...
for(const gamut of ['srgb','full']){
  const model=await factory({gamut,record,referenceWhiteNits:300});
  output.gamuts[gamut]={};
  ...
  for(const H of [60,273])for(const [R,L] of [[.1,.5],[.1,.9],[.2,.35],[.2,.9],[.5,1]]){
    const p=model.model.toPhysical({H,R,L}),G=model.model.transport.gray(L);
