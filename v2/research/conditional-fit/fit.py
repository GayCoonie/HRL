...
def main(argv=None):
    ...
    for key in ('reach_logits','reach_level_logits','level_logits'):
        seed[key]=np.array(seed[key],dtype=float)
    vector=np.concatenate((seed['reach_logits'].ravel(),seed['reach_level_logits'].ravel(),seed['level_logits'].ravel()))
    ...
    if args.start_record is not None:
        ...
        if prior['schema']!=seed['schema'] or prior['knots']!=seed['knots'] or prior['harmonics']!=seed['harmonics']:
    ...
    def assess(x, diagnostics=False):
        ...
        regularizer=.00006*anp.mean((coeff*frequency)**2)
