...
import fs from 'node:fs';import {createSpectralTonalHRL} from './index.mjs';import {createMappedHRL} from '../mapped-012/import.mjs';import {XYZ_TO_SRGB,mul3,encodeSRGB} from '../../lib/srgb-triangles.mjs';
const names=process.argv.slice(2),W=191,T=220,rows=[];if(!names.length)names.push('parent','boundary','balanced','metric');
for(const gamut of ['srgb','full'])for(const name of names){
 const dir=fs.existsSync(new URL(`results/${name}.json`,import.meta.url))?'results':'trials';const rec=['parent','boundary'].includes(name)?null:JSON.parse(fs.readFileSync(new URL(`${dir}/${name}.json`,import.meta.url)));const m=name==='parent'?await createMappedHRL({gamut,checkpoint:'balanced'}):await createSpectralTonalHRL({gamut,record:rec,checkpoint:name});
