...
const ids=process.argv.slice(2);if(!ids.length)ids.push('parent','boundary','balanced','metric');
...
for(const name of ids){out.models[name]={};for(const gamut of ['srgb','full']){
 const model=name==='parent'?await createMappedHRL({gamut,checkpoint:'balanced'}):await createSpectralTonalHRL({gamut,checkpoint:name}),rows=[];
