...
# Match the frozen IEEE-754 fused dot order explicitly, independent of BLAS CPU dispatch.
def dot3(v,w):return math.fma(float(v[2]),float(w[2]),math.fma(float(v[1]),float(w[1]),float(v[0])*float(w[0])))
n=[]
for a,b in zip(vertices,np.roll(vertices,-1,axis=0)):
 v=np.cross([*a,1-sum(a)],[*b,1-sum(b)])
 if dot3(v,w)<0:v=-v
 v=v/(dot3(v,w));n.append(v.tolist())
