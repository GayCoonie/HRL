...
import {createSpectralTonalHRL,ADOBE_RGB1998,sharedCoordinates as coordinates} from './index.mjs';
import {createSharedHRL} from '../shared-rl/source.mjs';
const root=new URL('./',import.meta.url),names=process.argv.slice(2);if(!names.length)names.push('balanced','metric');
...
for(const name of names){
 ...
 for(const gamut of ['srgb','full']){
  const m=await createSpectralTonalHRL({gamut,record:rec}),old=await createSpectralTonalHRL({gamut,checkpoint:'boundary'});ms.push(m);let error=0,neutral=0,hue=0,minDet=Infinity,maxCondition=0,minChannel=Infinity,maxChannel=-Infinity;
  for(let i=0;i<8192;i++){
   ...
   if(gamut==='srgb'){const rgb=m.toLinearRGB(q);minChannel=Math.min(minChannel,...rgb);maxChannel=Math.max(maxChannel,...rgb);}
  }
 ...
 const third=await createSpectralTonalHRL({gamut:ADOBE_RGB1998,record:rec});let e=0;for(let i=0;i<2048;i++){const rgb=[rand(),rand(),rand()],q=third.fromLinearRGB(rgb),back=third.toLinearRGB(q);e=Math.max(e,maxerr(rgb,back));}assert(e<1e-8);assert.deepEqual(third.definition.coefficients,rec.coefficients);out.untrainedThirdGamut={id:'a98-rgb',samples:2048,maxLinearRGBError:e,refitted:false};
 const s100=await createSpectralTonalHRL({gamut:'srgb',record:rec,referenceWhiteNits:100});for(let i=0;i<1024;i++){const rgb=[rand(),rand(),rand()];assert.deepEqual(s100.fromRGB(rgb),ms[0].fromRGB(rgb));}out.relative100vs300IdentitySamples=1024;
