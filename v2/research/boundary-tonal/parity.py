...
spec=importlib.util.spec_from_file_location('genfit',P/'fit-v2.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
...
for name in ['balanced','metric']:
 ...
 py=m.base.coords(q,C,D,shift).detach().numpy();pi=m.base.coords(q,C,D,shift,True).detach().numpy()
...
y=torch.tensor([.0001,.1,.5,.9],requires_grad=True);a=torch.tensor([.84,.7,.8,.84],requires_grad=True);result['implicitInverseGradientCheck']=bool(torch.autograd.gradcheck(m.base.InversePhi.apply,(y,a),eps=1e-6,atol=1e-7,rtol=1e-5));assert result['implicitInverseGradientCheck']
