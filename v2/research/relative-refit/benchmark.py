...
def main():
    ...
    cases=[
        {'id':'baseline-full-300-clip','mode':'baseline','gamut':'full','nits':300,'policy':'clip'},
        {'id':'refit-full-300-clip','mode':'refit','gamut':'full','nits':300,'policy':'clip'},
        {'id':'metric-full-300-clip','mode':'refit','checkpoint':'metric','gamut':'full','nits':300,'policy':'clip'},
        {'id':'refit-full-300-reject','mode':'refit','gamut':'full','nits':300,'policy':'reject'},
        {'id':'refit-full-100-clip','mode':'refit','gamut':'full','nits':100,'policy':'clip'},
        {'id':'refit-srgb-300-clip','mode':'refit','gamut':'srgb','nits':300,'policy':'clip'},
        {'id':'refit-srgb-100-clip','mode':'refit','gamut':'srgb','nits':100,'policy':'clip'},
    ]
    ...
    for gamut in ['full','srgb']:
        a=out['models'][f'refit-{gamut}-100-clip'];b=out['models'][f'refit-{gamut}-300-clip'];diffs=[]
...
def report(out):
    models=out['models'];names=['baseline-full-300-clip','refit-full-300-clip','refit-full-300-reject']
    lines=['# Corrected-solid R/L refit: ColorBench', '',
    ...
    lines+=['','## COMBVD, same ColorBench-prepared inputs','','| Model | Unweighted | Traditional weighted | Retained |','|---|---:|---:|---:|']
    for n,m in models.items():
        c=m['combvd'];lines.append(f"| {n} | {c['unweighted']:.6f} | {c['traditional_weighted']:.6f} | {c['retained']}/{c['total']} |")
    lines+=['','## Reference-white scale check','','Both contexts were actually evaluated with the same relative XYZ. The refit does not add an absolute-luminance appearance response. Identical scores do not select 300 over 100 nits.','','```json',json.dumps(out['nits_comparison'],indent=2),'```','','Only the five scored generation and sixteen measurement columns were run. See verification.json and visual diagnostics for the separate implementation and synthetic-path checks.']
