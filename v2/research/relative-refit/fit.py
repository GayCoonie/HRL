"""R/L-only fitting on the corrected relative-Y full solid.
...
97x97 source-XYZ grid supplies an explicitly approximate, differentiable
...
def main():
    ...
    start=np.array(rec['coefficients'][1])
    if args.start=='zero':start=np.zeros_like(start)
    elif args.start!='baseline':start=np.array(json.loads((HERE/args.start).read_text())['coefficients'][1])
    ...
    # All native coefficients and the globally shared neutral progression remain frozen.
    ...
    record=copy.deepcopy(rec);record['id']='HRL-0.9-RL-'+args.name;record['variant']=args.name;record['layers']=args.layers
    native=np.asarray(record['coefficients'][0]);native=np.pad(native,((0,args.layers-len(native)),(0,0),(0,0)))
    record['coefficients']=[native.tolist(),C.tolist()]
    record.pop('source_record_sha256',None);record.pop('scores',None)
    record['research']={'parent':'relative-domain corrected C1','fit':vars(args),'frozen':['carrier','physical solid','hue field and high-magnitude continuation','hue ring','source atlas','sRGB mapping','neutral shift'],'human_training':['COMBVD'],'not_used_for_training':['OSA-UCS','MacAdam1974','Xiao','threshold/tolerance data'],'proxy':f'Oklab of exact source XYZ cached on {cache["H"]}x{cache["N"]}x{cache["N"]} grid; bilinear interpolation only in optimization regularizer, never production runtime','cache_sha256':hashlib.sha256(cache_path.read_bytes()).hexdigest(),'metrics':stats,'evaluations':n,'seconds':time.monotonic()-t}
