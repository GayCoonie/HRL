...
import {createHueFairHRL} from './index.mjs';import {createGenTonalHRL} from '../gen-tonal-fit/index.mjs';
...
for(const name of names){out.models[name]={};for(const gamut of ['srgb','full']){
 const file=name==='parent'?'../gen-tonal-fit/results/balanced.json':`${fs.existsSync(new URL('results/'+name+'.json',root))?'results':'trials'}/${name}.json`,raw=fs.readFileSync(new URL(file,root)),record=JSON.parse(raw);
 const m=name==='parent'?await createGenTonalHRL({gamut,record}):await createHueFairHRL({gamut,record});const rows=[];
