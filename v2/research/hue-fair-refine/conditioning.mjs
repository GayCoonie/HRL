...
const names=['parent','balanced','gentle'],hues=[...new Set([...Array.from({length:72},(_,i)=>1+5*i),263,269,273,275,277,281,285,293])].sort((a,b)=>a-b),levels=[.001,.005,.02,.05,.1,.2,.35,.5,.65,.8,.95,.99,.999],ratios=[.001,.005,.02,.05,.1,.2,.35,.5,.65,.8,.9,.95,.99,.999];
...
for(const name of names){const raw=fs.readFileSync(new URL(name==='parent'?'../gen-tonal-fit/results/balanced.json':`results/${name}.json`,import.meta.url)),record=JSON.parse(raw),f=q=>sharedCoordinates(q,record),values=[];let worst=null,minDet=Infinity;
