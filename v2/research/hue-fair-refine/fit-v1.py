...
def main():
    ...
    for key,default,typ in [('name',None,str),('start','parent',str),('steps',700,int),('stride',2,int),('samples',97,int),('visual',.28,float),('fair',.6,float),('sheet',.08,float),('corner',1.,float),('guard',2.,float),('blue',.75,float),('layers',5,int),('mode','bilinear',str)]:
    ...
    parentpath=P/'../gen-tonal-fit/results/balanced.json';parent=json.loads(parentpath.read_text());startpath=parentpath if args.start=='parent' else P/f'trials/{args.start}.json';rec=json.loads(startpath.read_text())
    ...
    gridpath=P/'../gen-tonal-fit/results/grid.json';grid=json.loads(gridpath.read_text());HH,N=grid['H'],grid['N'];hh=list(range(0,HH,args.stride));hs=torch.tensor(hh)*360/HH;NH=len(hh)
    ...
    centres += [(L*u,L) for L in [.04,.08,.12,.18] for u in [.18,.4,.65,.84]]
    ...
    paircache=json.loads((P/'../shared-rl/results/cache.json').read_text());profiles=[]
    ...
    def sheet_terms(v):
        ...
        per=bend.mean(-1)+.25*torch.sqrt(bend.square().mean(-1)+1e-18)
        return per,bend
    pc=torch.tensor(parent['coefficients'][0]);pd=torch.tensor(parent['dark']['coefficients'])
    for g in ['srgb','full']:
        z=paircache['profiles'][g];image=torch.tensor(np.fromfile(P/f'../gen-tonal-fit/results/gen-grid-{g}.f64',dtype='<f8').reshape(HH,N,N,3)[hh]).permute(0,3,1,2).contiguous()
    ...
    def assess():
        ...
        for p in profiles:
            ...
            metric_guard=F.relu(st-p['base_stress2']).square()
            loss=st+args.visual*riskagg(risk)+args.sheet*riskagg(sheet)+args.corner*retreat+args.guard*guard+2*regret+10*metric_guard
            total=total+.5*loss
            stats[p['gamut']]={k:float(x.detach()) for k,x in {'stress':100*st.clamp_min(0).sqrt(),'pathRisk':risk.mean(),'worstHue':risk.max(),'bluePathRisk':risk[((hs-273+180)%360-180).abs()<23].mean(),'sheetBending':sheet.mean(),'worstSheet':sheet.max(),'blueSheet':sheet[((hs-273+180)%360-180).abs()<23].mean(),'retreat':retreat,'darkGuard':guard,'regret':regret}.items()}
        return total+1e-7*(C*freq).square().mean()+1e-6*D[1:].square().mean(),stats
    start=time.monotonic();calls=0;best=None;log=(P/f'trials/{args.name}.jsonl').open('w')
    def save(best):
        loss,cc,dd,stats,evaluation=best;r=copy.deepcopy(rec);r.update(id='HRL-0.12-hue-fair-'+args.name,variant=args.name,layers=args.layers,coefficients=[cc.tolist()]);r['dark']['coefficients']=dd.tolist()
        r['research']={'ruler':'helmlab-1.0.0-genspace','human_training':['COMBVD'],'parent':'0.11 Gen tonal balanced','parent_sha256':hashlib.sha256(parentpath.read_bytes()).hexdigest(),'start':str(startpath.relative_to(P)),'start_sha256':hashlib.sha256(startpath.read_bytes()).hexdigest(),'fit':vars(args),'grid_record':grid,'seed':180917,'per_gamut_objective_weights':[.5,.5],'synthetic':'Per-hue weighted mean/RMS path risk; equilateral Hessian/gradient bending; black/white corner retreats; parent dark-edge and good-hue guards','sheet_stencils':len(centres),'sheet_reference_scale':1/32,'blue_window':{'centre':273,'sigma':20,'amplitude':args.blue,'basis':'User-reported region, not observer dataset'},'stats':stats,'best_loss':loss,'best_evaluation':evaluation,'evaluations':calls,'seconds':time.monotonic()-start,'versions':{'torch':torch.__version__,'numpy':np.__version__},'data_exposure':'Prior development exposed evaluation datasets; no new held-out result used in this loss.'}
