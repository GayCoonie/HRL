"""Original scored Python ColorBench judges on frozen HRL with ordinary mapped input.
No tuning, tensor-unit edits, replacement derivative tests or extra unscored suites.
"""
...
def main():
    ...
    out={'methodology':METHODOLOGY_VERSION,'base_commit':'3aff53aaf976c41874ec68fd583084ba1f08c6f3','colorbench_commit':CB_SHA,'pool_commit':POOL_SHA,
      'scope':'5 scored generation and 16 scored measurement columns; no tuning or boundary upgrade',
      'upstream_source_provenance':provenance,'input_policy':'mapped-012-v1','judge_functions_unchanged':True,'forward_helper_unchanged':True,
      ...
      'mapping_policy':'Original 0.12 physical cone projection and chromaticity-preserving Y ceiling, plus explicit native linear-sRGB channel clipping when required.',
    ...
    with gzip.open(R/'point-event-masks.jsonl.gz','wt') as masklog:
        for gamut in ['srgb','full']:
            for mode in ['parent','balanced','gentle']:
                ...
                try:
                    ...
                    old=historical['models'][mode+'-'+gamut]['combvd'];common=np.array(old['retained_indices']);widx=np.repeat(common,weights[common])
                    ...
                    assert abs(m['historical_common_pairs']['weighted']-old['traditional_weighted'])<1e-9
                    if gamut=='full':assert m['combvd']['mapped_pairs']==0
                    if gamut=='full' and mode!='parent':
                        previous=historical['models'][mode+'-full-clip'];diff=[]
                        for board in ['generation','measurement']:
                            for k,z in m[board].items():diff.append(abs(z['score']-previous[board][k]['score']))
                        m['previous_mapped_reproduction_max_score_difference']=max(diff);assert max(diff)<1e-9
