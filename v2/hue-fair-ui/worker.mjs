import {createGenTonalHRL} from '../research/gen-tonal-fit/index.mjs';
import {createHueFairHRL} from '../research/hue-fair-refine/index.mjs';
const ids=['parent','balanced','gentle'],cache=new Map();
async function models(gamut){
 if(!cache.has(gamut))cache.set(gamut,Promise.all(ids.map(checkpoint=>checkpoint==='parent'?createGenTonalHRL({gamut,checkpoint:'balanced',overflow:'reject',imaginary:'reject'}):createHueFairHRL({gamut,checkpoint,overflow:'reject',imaginary:'reject'}))).catch(e=>{cache.delete(gamut);throw e;}));
...
function strip(model,H,axis,fixed){
 ...
 for(let x=0;x<width;x++){
  const t=x/(width-1),q=axis==='white'?{H,R:fixed*(1-t),L:t+fixed*(1-t)}:axis==='edge'?{H,R:fixed*t,L:t}:axis==='reach'?{H,R:fixed*t,L:fixed}:{H,R:fixed,L:fixed+(1-fixed)*t},out=display(model.toXYZ(q));
...
async function run(){
 ...
 try{while(latest){
  ...
  for(let i=0;i<ms.length;i++){
   ...
   const reach=strip(ms[i],job.H,'reach',job.fixedL),level=strip(ms[i],job.H,'level',job.fixedR),edge=strip(ms[i],job.H,'edge',1),near=strip(ms[i],job.H,'edge',.9),white=strip(ms[i],job.H,'white',.65);
   self.postMessage({type:'render',id:job.id,gamut:job.gamut,H:job.H,index:i,tri,reach,level,edge,near,white},[tri.pixels.buffer,tri.xyzs.buffer,tri.mask.buffer,reach.pixels.buffer,level.pixels.buffer,edge.pixels.buffer,near.pixels.buffer,white.pixels.buffer]);
  }
