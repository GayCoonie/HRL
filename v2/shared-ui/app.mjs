const $=id=>document.getElementById(id),ids=['baseline','balanced','metric'],names=['Previous separate balanced','Shared / dark-edge balanced','Shared / metric-leaning'];
...
for(let i=0;i<3;i++)$('panels').insertAdjacentHTML('beforeend',`<section class="panel" id="panel${i}"><header><h2 id="title${i}">${names[i]}</h2><button class="select-panel" data-select="${i}">Pick here</button></header><div class="score" id="score${i}"></div><div class="vertex"><span>White</span><span>Vivid →</span></div><div class="canvas-wrap"><canvas class="triangle" id="tri${i}" tabindex="0" aria-label="${names[i]} color triangle"></canvas><span class="cursor" id="cursor${i}" hidden></span></div><div class="vertex"><span>Black</span></div><p class="gamut" id="gamut${i}"></p><div class="reading" id="read${i}">Move over the triangle to inspect a color; click to select it.</div><div class="barlabel" id="rlabel${i}"></div><canvas class="bar" id="reach${i}"></canvas><div class="barlabel" id="llabel${i}"></div><canvas class="bar" id="level${i}"></canvas><div class="barlabel">Black → vivid: R = L</div><canvas class="bar edge" id="edge${i}"></canvas><div class="barlabel">Near edge: R = 0.9 L</div><canvas class="bar edge" id="near${i}"></canvas><div class="panel-actions"><button data-save="${i}">Save triangle PNG</button><span class="hint" id="modelTag${i}"></span></div></section>`);
...
function updateControls(){
 ...
 for(let i=0;i<3;i++){$('panel'+i).classList.toggle('selected',ids[i]===state.checkpoint);$('title'+i).textContent=i===0?(state.gamut==='srgb'?'Previous separate balanced':'Previous separate balanced'):names[i];$('modelTag'+i).textContent=state.gamut==='srgb'?'native sRGB':'full gamut';}
 $('displayNotice').innerHTML=state.gamut==='srgb'?'<strong>Native sRGB, not a clipped full triangle.</strong> Every valid address is generated inside sRGB using its own vivid boundary and the same learned R/L math as the full realization. The first panel is the previous separate balanced fit.':'<strong>Your display cannot show the whole full gamut.</strong> Actual full-domain XYZ is explicitly clipped to sRGB for preview only. The mask exposes those pixels. The underlying coordinates and benchmark distances do not use that clipped preview.';
 moveCursors();
}
...
worker.onmessage=({data:d})=>{
 ...
 images.set(d.index,d);drawTriangle(d.index);paint($('reach'+d.index),d.reach);paint($('level'+d.index),d.level);paint($('edge'+d.index),d.edge);paint($('near'+d.index),d.near);$('read'+d.index).textContent=`H ${d.H.toFixed(2)}° · move over this rendered triangle for current XYZ.`;
...
function colorRecord(){if(!lastSample||lastSample.gamut!==state.gamut||lastSample.sampleId!==sampleId)return null;const item=lastSample.readings[ids.indexOf(state.checkpoint)];return{profile:'HRL-0.10-shared-RL',gamut:state.gamut,checkpoint:state.checkpoint,sharedCalibration:state.checkpoint!=='baseline',referenceWhite:'D65',referenceWhiteNits:300,coordinates:lastSample.q,relativeXYZ:item.xyz,displaySRGB8:item.rgb,displayClipped:item.outside};}
...
function updateEvidence(){
 ...
 for(let i=0;i<3;i++){const m=p.models[ids[i]];$('score'+i).textContent=`COMBVD ${m.weighted.toFixed(4)} weighted / ${m.unweighted.toFixed(4)} unweighted · ${m.pairs} pairs`;}
 const a=p.models.baseline,b=p.models.balanced;let rows='';
 for(const [key,title]of [['mean_cv','Within-ramp step variation'],['mean_step_jump','Neighboring step-size jumps']])rows+=`<tr><td>${title}</td><td>${(100*(b.visual.reach[key]/a.visual.reach[key]-1)).toFixed(1)}%</td><td>${(100*(b.visual.level[key]/a.visual.level[key]-1)).toFixed(1)}%</td></tr>`;
 $('progress').innerHTML=`<p><strong>${state.gamut==='srgb'?'Native sRGB':'Full gamut'}:</strong> balanced weighted STRESS <strong>${a.weighted.toFixed(6)} → ${b.weighted.toFixed(6)}</strong>, on the same ${b.pairs} pairs.</p><div class="scroll"><table><tr><th>Balanced change from previous calibration</th><th>Reach</th><th>Level</th></tr>${rows}</table></div>`;
 const edge=p.edge;if(edge)$('progress').insertAdjacentHTML('beforeend',`<p>At R = L = 0.25, mean own-endpoint-normalized Oklab lightness changes from <strong>${edge.baseline.toFixed(4)} to ${edge.balanced.toFixed(4)}</strong>. This is a dark-edge diagnostic, not the definition of Level.</p>`);
 const board=$('board').value,keys=Object.keys(p.models.baseline[board]);if(board==='measurement')keys.sort((x,y)=>{const order=['bfd','leeds','witt','rit','macadam'];return (order.includes(x)?order.indexOf(x):9)-(order.includes(y)?order.indexOf(y):9);});
 const table=document.createElement('table'),head=table.insertRow();for(const text of ['Dataset','Previous','Balanced','Metric']){const th=document.createElement('th');th.textContent=text;head.appendChild(th);}
 for(const key of keys){const tr=table.insertRow();tr.insertCell().textContent=labels[key]||key;for(const id of ids){const v=p.models[id][board][key],td=tr.insertCell();td.textContent=v.score===null?'N/A':v.score.toFixed(6)+(v.exact?'':' †');td.tabIndex=0;td.title=`${v.exact?'Complete unchanged-input support':'Incomplete or mapped-input support'}. ${v.mapped||0} inputs mapped; ${v.rejected||0} rejected.${v.retained!==undefined?' Retained '+v.retained+' of '+v.total+' pairs.':''}`;if(!v.exact)td.className='support';}}
...
fetch('research/shared-rl/results/site-data.json').then(r=>{if(!r.ok)throw Error('Evidence unavailable: '+r.status);return r.json();}).then(d=>{evidenceData=d;updateEvidence();}).catch(e=>{$('progress').textContent=e.message;});
